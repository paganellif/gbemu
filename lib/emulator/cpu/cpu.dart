...
class CPU {
  ...
  CPU({required this.cartridge}) {
    pointers = List<int>.filled(2, 0);

    mmu = cartridge.createController(this)!;
    ppu = PPU(cpu: this);
    registers = Registers(cpu: this);
  }
  ...
  void updateInterrupts(int delta) {
    ...
    if (divCycle >= 256) {
      divCycle -= 256;
      mmu.writeRegisterByte(MemoryRegisters.DIV,
  ...
  void setInterruptTriggered(int interrupt) {
    mmu.writeRegisterByte(
        ...
        mmu.readRegisterByte(MemoryRegisters.TRIGGERED_INTERRUPTS) |
            interrupt);
  }
  ...
  void execute() {
    if (halted) {
      if (mmu.readRegisterByte(MemoryRegisters.TRIGGERED_INTERRUPTS) ==
          0) {
        clocks += 4;
      }
