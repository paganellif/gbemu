...
import 'cpu.dart';
import 'package:gbemu/emulator/memory/cartridge.dart';
...
class Registers {
  ...
  late List<int> registers;

  int get a{return registers[A];}
  int get b{return registers[B];}
  int get c{return registers[C];}
  int get d{return registers[D];}
  int get e{return registers[E];}
  int get f{return registers[F];}
  int get h{return registers[H];}
  int get l{return registers[L];}

  set a(int value){registers[A] = value;}
  set b(int value){registers[B] = value;}
  set c(int value){registers[C] = value;}
  set d(int value){registers[D] = value;}
  set e(int value){registers[E] = value;}
  set f(int value){registers[F] = value;}
  set h(int value){registers[H] = value;}
  set l(int value){registers[L] = value;}

  /// Pointer to the CPU object
  CPU cpu;

  Registers({required this.cpu}) {
    registers = List<int>.filled(8, 0);
  }
  ...
  int getRegister(int r) {
    ...
    if(r == H) {return h;}
    if(r == L) {return l;}
    if(r == 0x6) {return cpu.mmu.readByte(getRegisterPair(HL));}
  ...
  void setRegister(int r, int value) {
    ...
    else if(r == 0x6) {cpu.mmu.writeByte(getRegisterPair(HL), value);}
  }

  /// Fetches the world value of a registers pair, r is the register id as encoded by opcode (PUSH_rr).
  /// Returns the value of the register
  int getRegisterPair(int r) {
    if(r == BC) {return (b << 8) | c;}
    if(r == DE) {return (d << 8) | e;}
    if(r == HL) {return (h << 8) | l;}
    if(r == AF) {return (a << 8) | f;}
  ...
  int getRegisterPairSP(int r) {
    ...
    if(r == DE) {return (d << 8) | e;}
    if(r == HL) {return (h << 8) | l;}
    if(r == Registers.SP) {return cpu.sp;}
  ...
  void setRegisterPair(int r, int hi, int lo) {
    ...
    if(r == BC) {b = hi; c = lo;}
    else if(r == DE) {d = hi; e = lo;}
    else if(r == HL) {h = hi; l = lo;}
    else if(r == AF) {a = hi; f = lo & 0xF;}
  }
  ...
  bool getFlag(int flag) {
    ...
    if(flag == 0x4) {return (f & ZERO) == 0;}
    if(flag == 0x5) {return (f & ZERO) != 0;}
    if(flag == 0x6) {return (f & CARRY) == 0;}
    if(flag == 0x7) {return (f & CARRY) != 0;}
