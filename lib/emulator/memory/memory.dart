  ...
  void writeIO(int address, int value) {
    ...
    // Start H-DMA transfer
      ...
      if (cpu.cartridge.gameboyType != GameboyType.CLASSIC) {
        ...
        int source = ((registers[0x51] & 0xff) << 8) |
            (registers[0x52] & 0xF0);
        int destination = ((registers[0x53] & 0x1f) << 8) |
            (registers[0x54] & 0xF0);
    ...
    // OAM DMA transfer
      ...
      if (((registers[MemoryRegisters.TAC] ^ value) & 0x03) != 0) {
        cpu.timerCycle = 0;
        registers[MemoryRegisters.TIMA] =
            registers[MemoryRegisters.TMA];
      }
      ...
      if (value == 0x81) {
        ...
        if (Config.printSerialCharacters) {
          if (kDebugMode) {
