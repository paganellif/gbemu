...
class PPU {
  ...
  void reset() {
    ...
    bgPalettes = List<Palette>.filled(8, GBCPalette(List<int>.filled(4, 0)));
    spritePalettes = List<Palette>.filled(8, GBCPalette(List<int>.filled(4, 0)));
    ...
    if (cpu.cartridge.gameboyType == GameboyType.COLOR) {
      gbcBackgroundPaletteMemory
          .fillRange(0, gbcBackgroundPaletteMemory.length, 0x1f);
      ...
      PaletteColorsRow colors =
          PaletteColors.instance.getByHash(cpu.cartridge.checksum);

      bgPalettes[0] =
          GBPalette(cpu, colors.bg, MemoryRegisters.BGP);
      spritePalettes[0] =
          GBPalette(cpu, colors.obj0, MemoryRegisters.OBP0);
      spritePalettes[1] =
          GBPalette(cpu, colors.obj1, MemoryRegisters.OBP1);
    }
  }
  ...
  void setBackgroundPalette(int reg, int data) {
    ...
    int palette = reg >> 3;
    updatePalette(gbcBackgroundPaletteMemory, bgPalettes[palette],
        palette, (reg >> 1) & 0x3);
  }
  ...
  void setSpritePalette(int reg, int data) {
    ...
    int palette = reg >> 3;
    updatePalette(gbcSpritePaletteMemory, spritePalettes[palette],
        palette, (reg >> 1) & 0x3);
  }
  ...
  void tick(int cycles) {
    ...
    if (lcdCycles >= 456) {
      ...
      cpu.mmu.writeRegisterByte(
          MemoryRegisters.LY, (((ly + 1) % 154) & 0xff)
      );
      ...
      if (!isVBlank && cpu.mmu.dma != null) {
        cpu.mmu.dma?.tick();
      }

      cpu.mmu.writeRegisterByte(MemoryRegisters.LCD_STAT,
  ...
  void drawBackgroundTiles(List<int> data, int scanline) {
    ...
    for (int x = 0; x < 21; x++) {
      ...
      if (cpu.cartridge.gameboyType == GameboyType.COLOR) {
        int attributes =
            cpu.mmu.readVRAM(MemoryAddresses.VRAM_PAGESIZE + addressBase);
  ...
  void drawTile(
    ...
    for (int px = 0; px < 8; px++) {
      ...
      int paletteLower =
          ((cpu.mmu.readVRAM(address) & (0x80 >> logicalX)) >>
              (7 - logicalX));
